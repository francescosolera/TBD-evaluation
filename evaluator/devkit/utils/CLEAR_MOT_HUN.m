...
for i=1:Ngt
    ...
    % -------------- ADDED BY SOLERA+CALDERARA
    %compute the lentgth the trajectory has been correctly tracked
    
    H=hist(alltracked(gtframes,i),max(alltracked(gtframes,i))+1);
    H(1)=0;
    lengthCorrect=max(H);
    
    TLP(i)=lengthCorrect/gttotallength;
    % -------------- ADDED BY SOLERA+CALDERARA
end
...
metricsInfo.TLenghts_Percentage=TLP;        % -------------- ADDED BY SOLERA+CALDERARA
end 
